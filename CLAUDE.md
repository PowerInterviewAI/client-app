...
**`/main` is not a destination, and nothing offers it as one.** It is where a running session is, and the only two ways onto it are starting one and returning to one already running - both of which the home screen names. The command palette listed it under "Go to" beside Account and Configuration, which sent an idle user to a bar whose every control is disabled with the sole working one being the way back.
